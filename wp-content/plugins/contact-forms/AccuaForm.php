...
class AccuaForm extends Form {
  ...
  public function render($returnHTML = false) {
    ...
    if (!empty($this->accua_ajax)) {
      $ajax_url = json_encode(admin_url('admin-ajax.php?action=accua_form_submit'), JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP);
      $submit_fail_message = json_encode(__('Unable to submit the form, please retry', 'accua-form-api'), JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP);
      $required_message = json_encode(__('Please fill in all required fields', 'accua-form-api'), JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP);
      $valid_mail_message = json_encode(__('You have to enter a valid email address where required', 'accua-form-api'), JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP);
      $js_buildid = preg_replace('/[^a-zA-Z0-9_]/m','_',$this->buildID);
      $formid = $this->getId();
      $hostname = json_encode($_SERVER['SERVER_NAME'], JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP);

      $post_url = json_encode($this->stats['url'], JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP);
