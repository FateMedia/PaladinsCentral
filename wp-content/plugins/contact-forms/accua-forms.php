...
function accua_forms_menu(){
  $dashboard_admin_page=add_menu_page('Wordpress Contact Forms by Cimatti', 'Forms', 'manage_options', 'accua_forms', 'accua_dashboard_page', ACCUA_FORMS_DIR_URL.'img/cimatti-16-icon.png');
  add_action('load-'.$dashboard_admin_page, 'accua_dashboard_page_head');

  add_submenu_page('accua_forms', 'Wordpress Contact Forms by Cimatti', 'Dashboard', 'manage_options', "accua_forms", 'accua_dashboard_page');
  ...
  $form_add_page = add_submenu_page('accua_forms', __('Add new form', 'accua-form-api'), __('Add new', 'accua-form-api'), 'manage_options', "accua_forms_add", 'accua_forms_add_page');
  ...
  $form_submissions_page =  add_submenu_page('accua_forms', __('Forms submissions', 'accua-form-api') , __('Submissions', 'accua-form-api'), 'manage_options', "accua_forms_submissions_list", 'accua_forms_submissions_list_page');
  add_action('admin_head-'.$form_submissions_page, 'accua_forms_submissions_list_page_head');

  $form_fields_page = add_submenu_page('accua_forms', __( 'Form fields', 'accua-form-api'), __('Fields', 'accua-form-api'), 'manage_options', "accua_forms_fields", 'accua_forms_fields_page');
  ...
  $settings_page = add_submenu_page('accua_forms', __( 'Default Forms settings', 'accua-form-api'), __('Settings', 'accua-form-api'), 'manage_options', "accua_forms_settings", 'accua_forms_settings_page');
...
jQuery(function($){
  var data = <?php echo json_encode($data, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP); ?> ;
  var options = {
      xaxis: {
...
function accua_forms_edit_page_head_styles() {
  ...
  wp_enqueue_style( 'accua-forms-admin', plugins_url('accua-forms-admin.css', __FILE__), array('wp-color-picker'), '2');
}
...
function accua_forms_edit_page_head_scripts() {
  ...
  wp_enqueue_script('jquery-ui-draggable');
  wp_enqueue_script('jquery-ui-droppable');*/
  wp_enqueue_script( 'accua-form-fields', plugins_url( 'form-fields.js' , __FILE__ ), array( 'jquery-ui-sortable', 'jquery-ui-draggable', 'jquery-ui-droppable' ), '2' );
...
function accua_forms_edit_page($fid) {
  ...
  wp_enqueue_script('accua_tabs', plugins_url('accua_tabs.js', __FILE__ ), array( 'jquery' ), '1');
   ...
   jQuery(function($){
    ...
    <div id="accua_tabs">
      <div id="save_settings_top" class="accua_forms_save_settings_top">
        <form id="delete_form" action="admin.php?page=accua_forms_list" method="POST" onsubmit="return confirm(<?php echo htmlspecialchars(json_encode(__('Do you really want to delete this form?', 'accua-form-api'), JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP), ENT_QUOTES); ?>);">
        ...
        <input class="button button-primary button-large accua_form_save_settings_button" id="accua_form_save_settings" type="button" value="<?php echo htmlspecialchars(__( 'Save settings', 'accua-form-api'), ENT_QUOTES); ?>" />
...
$types = array(
  ...
  'post-select' => __( 'Post select', 'accua-form-api'),
...
<?php
  ...
  if($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['accua_form_save_form_settings'])) {
    ...
    foreach($empty_captcha_data as $key=>$val){
      $captcha_data[$key] = $post[$key];
    }
 ...
 <?php
  ...
  <div class="metabox-holder accua-forms-metabox-holder">
   ...
   <div class="metabox-holder accua-forms-metabox-holder">
     ...
     <div class="postbox ">
       <h3 class="hndle"><span><?php _e( 'reCaptcha settings', 'accua-form-api'); ?></span></h3>
       <div class="inside" id="dashboard_right_now">
         <p><?php _e( 'If you leave the key fields blank, old reCAPTCHA 1.0 is used with universal keys', 'accua-form-api');?></p>
         <p><?php echo strtr(__('If you want to use reCAPTCHA 2.0, please register on %REGISTERURL% and enter the keys for this site in the following fields', 'accua-form-api'), array('%REGISTERURL%' => '<a href="https://www.google.com/recaptcha" target="_blank">google.com/recaptcha</a>'));?></p>
         <div id="accua_"><?php _e('Site key', 'accua-form-api');?> : <input name="recaptcha_public_key" class="accua_form_value" type="text" value="<?php echo htmlspecialchars($captcha_data['recaptcha_public_key'], ENT_QUOTES) ?>" />
         </div>
         <div id="accua_"><?php _e('Secret key', 'accua-form-api');?> : <input name="recaptcha_private_key" class="accua_form_value" type="text" value="<?php echo htmlspecialchars($captcha_data['recaptcha_private_key'], ENT_QUOTES) ?>" />
         </div>
         <div id="accua_"><?php _e('Force old reCAPTCHA v1.0', 'accua-form-api');?> : <input type="checkbox" name="recaptcha_force_v1" class="accua_form_value" type="text" value="1" <?php if($captcha_data['recaptcha_force_v1']) { echo 'checked="checked"'; } ?> /><br />
           <small><?php _e( "Check if you want to use old reCAPTCHA 1.0 with the keys you provided", 'accua-form-api');?>.</small>
         </div>
       </div>
     </div>
   </div>
      ...
      foreach ($form_data['fields'] as $istance_data) {
        ...
        switch ($field_data['type']) {
          ...
          case 'captcha':
              ...
              if ($captcha_use_v1) {
                $element = new AccuaForm_Element_Captcha ($istance_data['label'], $captcha_properties);
              } else {
...
function accua_forms_aggregate_submitted_data(&$replace_map, $params = array()) {
  ...
  if (!empty($params['json'])) {
    $replace_map['__submitted_json'] = json_encode($replace_map['__submitted_json_raw'], JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP);
  }
        ...
        switch ($field_data['type']) {
          ...
          case 'radio':
            $el = $form->getElementByName($istance_id);
	          ...
	          $replace_map['__label_'.$istance_data['istance_id']] = implode(", ", $labels);
...
function &accua_get_pages($args = '') {
  ...
  $defaults = array(
      ...
      'meta_key' => '', 'meta_value' => '',
      'meta_value_lt' => '', 'meta_value_gt' => '',
      'meta_value_le' => '', 'meta_value_ge' => '',
      'meta_value_like' => '', 'meta_value_format' => 'string',
